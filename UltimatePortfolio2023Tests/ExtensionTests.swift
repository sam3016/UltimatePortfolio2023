...
final class ExtensionTests: BaseTestCase {
    ...
    func testIssueTagsUnwrap() {
        ...
        XCTAssertEqual(issue.issueTags.count, 0, "A new issue should have no tag.")
    ...
    func testIssueSortingIsStable() {
        ...
        XCTAssertEqual([issue3, issue1, issue2], sorted, "Sorting issue arrays should use name then creationDate.")
    }
    ...
    func testTagSortingIsStable() {
        ...
        tag2.name = "B Tag"
        tag2.id = UUID(uuidString: "FFFFFFFF-E68F-4655-90AB-4160411BBAF1")
        ...
        XCTAssertEqual([tag3, tag1, tag2], sortedTags, "Sorting tag arrays should use name then uuid.")
    }

    func testBundleDecodingAwards() {
        let awards = Bundle.main.decode("Awards.json", as: [Award].self)
        XCTAssertFalse(awards.isEmpty, "Awards should decode to a non-empty array.")
    }
